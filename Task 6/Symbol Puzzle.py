...
class Puzzle():
    ...
    def SavePuzzle(self):
        file_name = input("Enter a name to save the file under: ") + ".txt"
        
        encoded = []

        # Symbols
        encoded.append(len(self.__AllowedSymbols))
        for symbol in self.__AllowedSymbols:
            encoded.append(symbol)

        # Patterns
        encoded.append(len(self.__AllowedPatterns))
        for pattern in self.__AllowedPatterns:
            encoded.append(f"{pattern.GetSymbol()},{pattern.GetPatternSequence()}") # Get Symbol Method Created for This Purpose

        # Grid Encoding
        encoded.append(self.__GridSize)
        for cell in self.__Grid:
            cell_string = cell._Symbol + "," # GetSymbolsNotAllowed Created for this purpose
            for blocker in cell.GetSymbolsNotAllowed():
                if blocker in self.__AllowedSymbols: # Sometimes they're blank#
                    cell_string += blocker + ","
                
            encoded.append(cell_string)

        # Additional Bits
        encoded.append(self.__Score)
        encoded.append(self.__SymbolsLeft)

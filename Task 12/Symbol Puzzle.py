...
class Puzzle():
    def __init__(self, *args):
        ...
        # -2 means triggered already,
        #  0 means trigger this turn,
        # any other number is the length of time until trigger
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if self.swamp == -1: # If the swamp event hasn't been triggered yet
                swamp_trigger = random.randint(1,4)
                if swamp_trigger == 4:
                    self.swamp = random.randint(2,4)
                    print(f"A swamp event will trigger in {self.swamp} turns")

            elif self.swamp == 0: # Swamp this turn
                tiles_to_swamp = random.randint(1,4)
                print(f"A swamp event is occuring, covering {tiles_to_swamp} tiles")
                for i in range(tiles_to_swamp):
                    found = False
                    while not found:
                        index = random.randint(0, len(self.__Grid)-1)
                        tile = self.__Grid[index]
                        if tile._Symbol == "":
                            found = True
                    self.SwampThisCell(index)
                self.swamp = -2

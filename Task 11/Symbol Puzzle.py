...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Row = -1
            if self.levers > 0:
                use_lever = input("Would you like to use a lever? (Y/N): ").lower()
            else:
                use_lever = "n"
            Valid = False
            while not Valid:
            ...
            if use_lever == "y":
                if CurrentCell._Symbol == "@":
                    Index = (self.__GridSize - Row) * self.__GridSize + Column - 1
                    self.__Grid[Index] = Cell()
                    self.levers -= 1
                    print("Blocked cell was removed with a lever.")
                else:
                    print("Position did not contain a blocked cell, you still have a lever to use")
            else:

...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            print("Symbols Remaining: " + str(self.__SymbolsLeft))

            user_remove_decision = input("Would you like to remove a symbol? (Y/N)")
            Valid = True
            if user_remove_decision == "Y":
                try:
                    Row = int(input("Enter row number: "))
                    Column = int(input("Enter column number: "))
                except:
                    print("One or more of your inputs was invalid, continuing the game without removing a symbol")
                    Valid = False
                if Valid:
                    ...
                    if CurrentCell.CheckSymbolAllowed(Symbol):
                        ...
                        print("That is not a valid removal location, continuing the game without removing a symbol")
                        Valid=False
                    
            if user_remove_decision != "Y" or Valid == False:
                Row = -1
                Valid = False
                while not Valid:
                    try:

...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            deduction = self.GetSpaceLeftOnGrid()
            if deduction != None:
                print(f"You did not have enough symbols to place another symbol, leading to a penalty of {deduction} points and the game ending.")
    ...
    def GetSpaceLeftOnGrid(self):
        ...
        for y in range(self.__GridSize):
            for x in range(self.__GridSize):
                mismatch = self.CheckforMatchWithPattern(y,x, True)
                if lowest_mismatch == 0:
                    lowest_mismatch = mismatch
                else:
                    if mismatch != 0:
                        lowest_mismatch = min(lowest_mismatch, mismatch)
                        
        if lowest_mismatch >= self.__SymbolsLeft:
            deduction = self.__SymbolsLeft
            self.__Score -= deduction
            self.__SymbolsLeft = 0
            return deduction
        return None

    def CheckforMatchWithPattern(self, Row, Column, return_mismatch = False):
        lowest_mismatch = 0
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if not return_mismatch:
                            ...
                            if lowest_mismatch == 0:
                                lowest_mismatch = P.MatchesPattern(PatternString, CurrentSymbol, True)
                            else:
                                mismatch = P.MatchesPattern(PatternString, CurrentSymbol, True)
                                if mismatch != 0:
                                    lowest_mismatch = min(lowest_mismatch, P.MatchesPattern(PatternString, CurrentSymbol, True))
                except:
                    pass
        if not return_mismatch:
            return 0
        else:
            return lowest_mismatch
...
class Pattern():
    ...
    def MatchesPattern(self, PatternString, SymbolPlaced, return_mismatch = True):
        ...
        if mismatch >= 1 and not return_mismatch:
            return False
        elif mismatch == 0 and not return_mismatch:
            return True
        else:
            return mismatch
